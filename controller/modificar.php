<?php
    ...
    if(!empty($_POST["btnModificar"])){
        if(!empty($_POST["nombre"]) and !empty($_POST["apellido"]) and !empty($_POST["dni"]) and !empty($_POST["fecha"]) and !empty($_POST["correo"])){
            ...
            $sql = $conexion->query("UPDATE persona SET nombre = '$nombre', apellido = '$apellido', dni = '$dni', fechaNacimiento = '$fechaNacimiento', correo = '$correo' WHERE idPersona = $id");
            ...
            if($sql == 1){
                ...
                <div class='alert alert-danger d-flex align-items-center' role='alert'>
                    ...
                    <div>Error al Modificar los Datos</div>
                </div>";
            }
        } else {
            echo "
